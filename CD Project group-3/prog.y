...
expressions
	...
	| expressions XOR expressions {$$=typeassign($1,$3);}
	| IDENTIFIER {strcpy(temp,$1);$$=variablecheck(temp);}
	| NUM {$$=2;}
	| OP expressions CP {$$=$2;}
	| INC IDENTIFIER {strcpy(temp,$2);$$=variablecheck(temp);if($$!=2) fprintf(yyout,"ERROR: Increment on only Integers\n");}
	| DEC IDENTIFIER {strcpy(temp,$2);$$=variablecheck(temp);if($$!=2) fprintf(yyout,"ERROR: Decrement on only Integers\n");}
	| REAL {$$=3;}
	| CHARVALUE {$$=1;}
	;
